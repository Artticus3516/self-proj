...
Implement all 20 Tier 1 E2E test cases in Playwright under tests/tier1.spec.ts, ensuring they align with selector
contracts, build correctly, and pass lint checks.
...
- Targeted the real paths (e.g. `/services`, `/about`, `/contact`, `/admin/login`, `/admin`) while utilizing Playwright
  page locator selectors conforming strictly to the requested selector contracts (e.g. cookie consent, contact form,
  CRUD title/description/content).
