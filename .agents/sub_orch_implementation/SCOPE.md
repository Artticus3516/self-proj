...
- **Backend**: Supabase integration for CRUD data storage and admin auth. In "demo" integrity mode, a robust mock
  database client layer wrapped in the Supabase client interface is implemented, running deterministically without
  internet, but using proper env vars.
- **Traffic Tracking**: A GDPR & DPDP compliant cookie consent banner controlling custom client-side analytics, traffic
  logs persisted in the database after consent.
- **Security**: Next.js App Router route handlers and middleware (renamed to `src/proxy.ts` in Next.js 16) protecting
  `/admin/*` routes. Rate-limiting applied on `/api/auth/login` (admin login) yielding HTTP 429 after 5 repeated
