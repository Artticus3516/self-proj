...
- **What to build**: Add `@playwright/test` to devDependencies, run install, install playwright chromium browser, create
  `playwright.config.ts` (with `tests/` path and webServer on port 3000), write a basic spec in `tests/basic.spec.ts`,
  run and verify it.
- **Success criteria**: Playwright tests pass successfully via command line; a genuine end-to-end test executes and
  asserts the root page loads properly.
...
- C:\Users\aarav\GitHub\self-proj\.agents\worker_e2e_infra\handoff.md — Handoff report containing findings, commands,
  and outputs.
